...
export class Ball {
  constructor(
    ...
    this.direction = { x: 0.5, y: 0 }; //Ball speed when game starts
    this.acceleration = 0.1; //X speed multipler on successful block
    this.initial = { x: 0.5, y: 0 }; //speed reset on point score
    this.maximum = { x: 40, y: 1 }; //maximum speed
    this.inertialTransfer = 0; //multiplier for velocity.y transfer from paddle
    this.htmlElement = null; //element on HTML page
  }
}
